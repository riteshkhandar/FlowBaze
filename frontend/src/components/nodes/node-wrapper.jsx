...
export const NodeWrapper = ({ title, children, handles, id, icon: Icon }) => {
  ...
  return (
    <Card className="group relative w-[250px] p-5 rounded-xl border-2 border-orange-300 bg-card transition-all hover:shadow-[0_0_0_2px_#f97316]">
      <button
          ...
          {handle.label && (
            <span
              className={`${getLabelStyle(
                handle.position,
              )} text-xs text-muted-foreground whitespace-nowrap pointer-events-none`}
            >
              {handle.label}
            </span>
          )}
      ...
      <div className="mb-3 text-sm font-semibold text-card-foreground flex items-center gap-2">
